...
$("#exportToCSV").click(function () {
	...
	console.log(data);
    $('<form action="ajax/ajax.php?action=anno_info_export" method="POST" enctype="multipart/form-data">' +
...
$("#batchDel").click(function() {
	...
	if (confirm("确认要删除"+idList.length+"个任务吗？")) {
		$.ajax({
			type: "POST",
			url: "ajax/ajax.php?action=anno_info_batchDel",
			dataType: "json",
			data: {
				idList: idList
			},
...
function doQuery() {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=anno_info",
        dataType: "json",
        data: {
            taskName:taskName,
            matchFlag:matchFlag,
