...
var columns = [
    ...
    {id: "v_id", name: "商店ID", field: "v_id"},
    {id: "name", name: "商店中文名", field: "name" ,width:100},
    {id: "name_en", name: "商店英文名", field: "name_en" ,width:100},
...
function beforeAddWht() {
    $.ajax({
        url : 'ajax/ajax.php?action=shop_bw_list_getMaxID',
        type : 'POST',
        dataType: 'json',
...
$('#addBtn').click(function() {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=shop_bw_list_addWhite",
        dataType: "json",
        data: {
            id:id,
            name:name,
...
$('#addToBlack').click(function() {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=shop_bw_list_addBlack",
        dataType: "json",
        data: {idList:idList},
        success: function(data) {
            if (data.status == 0) {
...
$('#removeBlack').click(function() {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=shop_bw_list_removeBlack",
        dataType: "json",
        data: {idList:idList},
        success: function(data) {
            if (data.status == 0) {
...
$("#annoBtn").click(function () {
	...
	$.ajax({
		type: "POST",
		url: "ajax/ajax.php?action=shop_bw_list_anno",
		dataType: "json",
		data: {
			task:task,
...
$("#import1Btn").click(function() {
    ...
    $.ajax({
        url : 'ajax/ajax.php?action=shop_bw_list_import1',
        type : 'POST',
        dataType: 'json',
        data : formData,
...
$("#import2Btn").click(function() {
    ...
    $.ajax({
        url : 'ajax/ajax.php?action=shop_bw_list_import2',
        type : 'POST',
        dataType: 'json',
        data : formData,
        processData: false,  // tell jQuery not to process the data
...
$("#editBtn").click(function() {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=shop_bw_list_edit",
        dataType: "json",
        data: {
            v_id:v_id,
            name:name,
...
function batchOpt(opt) {
	...
	if (confirm("确认将"+idList.length+"个购物状态改为"+optStr+"吗？")) {
		$.ajax({
			type: "POST",
			url: "ajax/ajax.php?action=shop_bw_list_batch",
			dataType: "json",
			data: {
				idList: idList,
				opt:opt
...
$("#exportToCSV").click(function () {
    ...
    for (var i = 0; i < idList.length; ++i) {
        ...
        for (var j = 0; j < sourceData.length; ++j) {
            if (idList[i] == sourceData[j].id) {
				...
				tmp["图片数"] = sourceData[j].image_list!=null&&sourceData[j].image_list!=''?sourceData[j].image_list.split('|').length:'0';
    ...
    $('<form action="ajax/ajax.php?action=shop_bw_list_export" method="POST" enctype="multipart/form-data">' +
...
function makeTable(data) {
    ...
    data.forEach(function(line) {
        var obj = {
			...
			"image_num":line.image_list!=null&&line.image_list!=''?line.image_list.split('|').length:'0',
            "tag":line.norm_tagid,
...
function doPureQuery() {
    $.ajax({
        type: "POST",
        url: "ajax/ajax.php?action=shop_bw_list",
        dataType: "json",
        data: {
            viewName:viewName,
			matchFlag:matchFlag,
